...
function first(value: unknown, keys: string[]): string | undefined {
...
function strategyIdFromEditUrl(url: URL): string | undefined {
...
function formFromHtml(html: string): Editor {
...
export class JoinQuantClient implements RemoteClient {
  constructor(private readonly http: SafeHttp) {}
  private async editor(id: string): Promise<Editor> {
  ...
  async authStatus(): Promise<Record<string, unknown>> {
    ...
    const explicitLoggedOut = /\bg_isLogin\s*=\s*false\b|用户名或密码错误/.test(
      response.body,
    );
    const strategyResource = /\/algorithm\/index\/(?:edit|new)(?:[/?"'])/.test(
      response.body,
    );
  ...
  private async resultContext(
    publicId: string,
  ): Promise<{ csrf: Fields; internalId: string; parameters: Fields }> {
  ...
  private async resultCall(
    route: string,
    id: string,
    offset = 0,
    cachedContext?: { csrf: Fields; internalId: string; parameters: Fields },
  ): Promise<unknown> {
  ...
  async backtestStatus(id: string): Promise<Record<string, unknown>> {
  ...
  async backtestResult(id: string): Promise<Record<string, unknown>> {
    ...
    let transactionMeta: Record<string, unknown> | undefined;
    if (status.state === "success") {
      ...
      const dataObj = transactionData as Record<string, unknown>;
