...
function idOf(value: unknown, keys: string[]): string | undefined {
...
export class SuperMindClient implements RemoteClient {
  ...
  async backtestStatus(id: string): Promise<Record<string, unknown>> {
    const info = objectResult(
      await this.call("/platform/backtest/queryinfo/", { backTestId: id }),
    );
    ...
    const state =
      raw === "SUCCESS"
        ...
        : ["FAIL", "FAILED", "CANCEL", "CANCELLED"].includes(raw)
          ? "failed"
          : ["COMPILING", "RUNNING", "WAITING", "PENDING", "QUEUED"].includes(
                raw,
              )
            ? "running"
            : "unknown";
