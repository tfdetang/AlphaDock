...
function platformOption(): Option {
...
function dateRange(start: string, end: string): void {
...
export function buildProgram(): Command {
    ...
    .action(
      async (
        ...
        try {
          ...
          if (owned) {
            const cleanedUp = await deleteKernel(session, id, http, jar).catch(
              () => false,
            );
            if (!cleanedUp)
              throw new CliError(
      ...
      async (
        ...
        const result = await client.createStrategy(
          ...
          async (id) => {
            ...
            if (
              !operation.value.strategyIds.includes(operation.value.strategyId)
            )
  ...
  const addRead = (name: "status" | "result") =>
    backtest
      .command(name)
      .addOption(platformOption())
  ...
  backtest
    ...
    .action(
      async (options: {
        ...
        await assertOutputAvailable(options.out);
        const { client } = await clientFor(options.platform);
        let latestState: unknown = "unknown";
    ...
    .action(
      (
        query: string,
        {
          platform,
          category: selected,
        }: { platform: Platform; category?: string },
      ) => {
        process.stdout.write(
  ...
  api
    ...
    .argument("<name>")
    .addOption(platformOption())
    .action((name: string, { platform }: { platform: Platform }) => {
      process.stdout.write(
