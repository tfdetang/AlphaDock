...
export function parseJsonSequence(text: string): unknown[] {
...
function flatten(values: unknown[]): CookieInput[] {
  const output: CookieInput[] = [];
  for (const value of values) {
    if (Array.isArray(value)) output.push(...(value as CookieInput[]));
    else if (
      value &&
      typeof value === "object" &&
      Array.isArray((value as { cookies?: unknown }).cookies)
    )
      output.push(...(value as { cookies: CookieInput[] }).cookies);
    else if (value && typeof value === "object")
      output.push(value as CookieInput);
    else
      throw new CliError(
        "COOKIE_FORMAT_INVALID",
        "auth",
...
export async function loadCookieJar(path: string): Promise<CookieJar> {
  ...
  for (const item of flatten(parseJsonSequence(text))) {
    ...
    const expires =
      typeof expiresRaw === "number" && expiresRaw > 0 && item.session !== true
