...
async function readBounded(response, maxBytes, diagnostics) {
    ...
    try {
        ...
        if (error instanceof CliError)
            throw error;
        await reader.cancel().catch(() => undefined);
        throw new CliError("RESPONSE_STREAM_FAILED", "transport", "Remote response body could not be read", diagnostics(error));
    }
    finally {
        reader.releaseLock();
    }
    ...
    async request(input, options = {}) {
        ...
        for (let count = 0; count <= redirects; count++) {
            ...
            try {
                ...
                throw new CliError("TRANSPORT_FAILED", "transport", "Remote request failed", transportDiagnostics(error, url, method, startedAt, count, "request"));
            }
            ...
            const body = await readBounded(response, options.maxBytes ?? 4_000_000, (error) => transportDiagnostics(error, url, method, startedAt, count, "response_body"));
