...
import { CliError, assertId, parseDate, parsePositive, publicError, } from "./errors.js";
import { configure, exclusiveOutput, assertOutputAvailable, updateJournal, } from "./storage.js";
import { loadCookieJar } from "./cookies.js";
import { clientFor } from "./platform.js";
import { notebookSession, createKernel, deleteKernel, executeKernel, notebookExecutionReport, notebookByteLimit, DEFAULT_NOTEBOOK_BYTES, } from "./jupyter.js";
...
function confirmed(value, code, text) {
    if (value !== true)
        throw new CliError(code, "input", text);
}
function dateRange(start, end) {
    parseDate(start, "start");
    parseDate(end, "end");
    if (start > end)
        throw new CliError("INVALID_DATE_RANGE", "input", "start must not be after end");
}
export function buildProgram() {
    const program = new Command()
        .name("alphadock")
        .version("0.1.0")
        .description("Safety-focused CLI for existing JoinQuant and SuperMind sessions")
        .showHelpAfterError()
        .exitOverride();
    const auth = program
        .command("auth")
    ...
    auth
        ...
        .requiredOption("--cookie-file <path>", "external Cookie/Playwright JSON file")
    ...
    notebook
        ...
        .requiredOption("--confirm-remote-execution", "acknowledge remote code execution")
        .action(async (file, options) => {
        confirmed(options.confirmRemoteExecution, "CONFIRMATION_REQUIRED", "--confirm-remote-execution is required");
        if (Number(Boolean(options.kernelId)) +
            Number(Boolean(options.temporary)) !==
            1)
            throw new CliError("KERNEL_SELECTION_REQUIRED", "input", "Choose exactly one of --kernel-id or --temporary");
        ...
        if (options.kernelId)
            assertId(options.kernelId, "kernel-id");
        ...
        if (id && !session.kernels.some((kernel) => kernel.id === id))
            throw new CliError("KERNEL_NOT_FOUND", "input", "The explicit kernel ID is not in the current session");
        if (options.temporary) {
            id = await createKernel(session, http, jar);
            owned = true;
        }
        if (!id)
            throw new CliError("KERNEL_SELECTION_REQUIRED", "input", "A kernel must be selected");
        try {
            ...
            if (owned) {
                const cleanedUp = await deleteKernel(session, id, http, jar).catch(() => false);
                if (!cleanedUp)
                    throw new CliError("EXECUTION_UNVERIFIED", "remote", `Execution was not verified; owned temporary kernel retained or cleanup unverified: ${id}`, error instanceof CliError ? error.diagnostics : undefined);
            }
            throw error;
        }
    });
    ...
    strategy
        ...
        .action(async (file, options) => {
        confirmed(options.confirmRemoteWrite, "CONFIRMATION_REQUIRED", "--confirm-remote-write is required");
        if (!options.name.trim() || options.name.length > 120)
            throw new CliError("INVALID_NAME", "input", "name must contain 1-120 characters");
        const code = await readFile(resolve(file), "utf8");
        if (!code.trim())
            throw new CliError("EMPTY_SOURCE", "input", "Python source file is empty");
        const { client, operation } = await prepareRemoteOperation("strategy-create", options.platform, { name: options.name }, code);
        operation.value.stage = "write-attempted";
        await updateJournal(operation.path, operation.value);
        const result = await client.createStrategy(options.name, code, async (id) => {
            operation.value.strategyId = assertId(id, "strategy-id");
            operation.value.strategyIds ??= [];
            if (!operation.value.strategyIds.includes(operation.value.strategyId))
                operation.value.strategyIds.push(operation.value.strategyId);
            operation.value.stage = "identity-preserved";
            await updateJournal(operation.path, operation.value);
        });
        operation.value.stage = "verified";
        await updateJournal(operation.path, operation.value);
        emit({ ok: true, operationId: operation.value.operationId, ...result });
    });
    ...
    backtest
        ...
        .addOption(new Option("--frequency <frequency>")
        .choices(["day", "minute"])
        .makeOptionMandatory())
        ...
        const { client, operation } = await prepareRemoteOperation("backtest-submit", options.platform, parameters);
        operation.value.stage = "submit-attempted";
        await updateJournal(operation.path, operation.value);
        const result = await client.submitBacktest(parameters, async (id) => {
    ...
    const addRead = (name) => backtest
        ...
        if (options.out) {
            ...
            emit({ ok: true, output: resolve(options.out), backtestId: id });
        }
        else
            emit(output);
    });
    ...
    backtest
        ...
        .action(async (options) => {
        const id = assertId(options.backtestId, "backtest-id");
        const timeout = Number(options.timeout), interval = Number(options.interval);
        if (!Number.isFinite(timeout) || timeout <= 0 || timeout > 86_400)
            throw new CliError("INVALID_TIMEOUT", "input", "timeout must be between 0 and 86400 seconds");
        if (!Number.isFinite(interval) || interval < 20 || interval > timeout)
            throw new CliError("INVALID_INTERVAL", "input", "interval must be at least 20 seconds and no greater than timeout");
        await assertOutputAvailable(options.out);
        const { client } = await clientFor(options.platform);
        let latestState = "unknown";
        const status = await waitForTerminal(async () => {
            const value = await client.backtestStatus(id);
            latestState = value.state;
            return value;
        }, timeout * 1000, interval * 1000);
        if (!status) {
            const output = {
                ok: false,
    ...
    const api = program
        .command("api")
        .description("Offline curated API subset (never loads credentials or uses network)");
    ...
    api
        ...
        process.stdout.write(`${renderCatalog(selectCatalog(platform, undefined, selected))}\n`);
    });
    api
        .command("search")
        .argument("<query>")
        ...
        process.stdout.write(`${renderCatalog(selectCatalog(platform, query, selected))}\n`);
    });
    api
        .command("show")
        .argument("<name>")
        ...
        process.stdout.write(`${renderCatalog(catalog.filter((entry) => entry.platform === platform && entry.name.toLowerCase() === name.toLowerCase()))}\n`);
    });
    return program;
}
export async function main(argv = process.argv) {
    try {
        ...
        const normalized = error instanceof CommanderError
            ? new CliError("CLI_USAGE", "input", error.message.replace(/^error:\s*/, ""))
            : error;
