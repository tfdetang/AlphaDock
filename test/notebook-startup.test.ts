...
const form =
  '<form method="post" action="/notebook/hub/spawn"><label><input name="profile" type="radio" value="py35" checked>Python 3.5 默认环境</label><label><input name="profile" type="radio" value="opaque38">Python 3.8 升级Python环境</label><input type="submit" value="Start"></form>';
const page = (body = form): HttpResponse => ({
  status: 200,
  url: new URL(origin + "/notebook/hub/spawn"),
  headers: new Headers(),
  body,
});
const state = (ready = false, pending: string | null = null) =>
  JSON.stringify({
    name: "fixture",
    server: ready ? "/notebook/user/fixture/" : null,
    pending,
    servers: {},
  });
const response = (body: string, status = 200, headers = {}) => ({
  status,
  ...
  await isolated(async (dir) => {
    ...
    const http = new SafeHttp("supermind", jar, async (url, init) => {
      if (init?.method === "POST") {
        ...
        assert.equal(
          new URLSearchParams(String(init.body)).get("profile"),
          "opaque38",
        );
        assert.equal(
          new Headers(init.headers).get("x-xsrftoken"),
          "fixture-token",
        );
        const files = await readdir(join(dir, "operations"));
        assert.equal(files.length, 1);
        const journal = JSON.parse(
          await readFile(join(dir, "operations", files[0]!), "utf8"),
        );
        ...
        return response("", 302, {
          location: "/notebook/hub/spawn-pending/fixture",
        });
      }
...
test("unknown POST outcomes and timeouts retain the lock across another invocation", async () => {
  for (const mode of ["dropped", "timeout", "307"] as const)
    await isolated(async (dir) => {
      let posts = 0;
      const jar = new CookieJar();
      const http = new SafeHttp("supermind", jar, async (_url, init) => {
        if (init?.method === "POST") {
          ...
          if (mode === "307")
            return response("", 307, { location: "/notebook/hub/spawn" });
          return response("", 202);
        }
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        await assert.rejects(
          startNotebookServer("supermind", page(), http, jar, controls),
          (error: unknown) => {
...
test("payment, resource choices, ambiguous profiles and foreign targets block without POST", async () => {
  for (const body of [
    ...
    form.replace(
      "</form>",
      '<select name="size"><option>GPU</option></select></form>',
    ),
    form.replace(
      "Python 3.8 升级Python环境",
      "Python 3.11(Beta) 升级Python环境",
    ),
    form.replace(
      "</form>",
      '<label><input type="radio" name="profile" value="duplicate">Python 3.8 升级Python环境</label></form>',
    ),
    form.replace(
      "/notebook/hub/spawn",
      "https://example.com/notebook/hub/spawn",
    ),
    ...
    await isolated(async () => {
      ...
      await assert.rejects(
        startNotebookServer("supermind", page(body), http, jar, controls),
        (error: unknown) =>
          error instanceof CliError && error.code === "SERVER_START_BLOCKED",
      );
...
test("existing pending startup is polled without submitting or claiming a Python version", async () => {
  await isolated(async () => {
    ...
    const result = await startNotebookServer(
      "supermind",
      page(),
      http,
      jar,
...
test("a delayed contender rechecks after lock acquisition and cannot submit using stale stopped state", async () => {
  await isolated(async () => {
    ...
    const fastJar = new CookieJar(),
      slowJar = new CookieJar();
    ...
    const delayed = startNotebookServer(
      "supermind",
      page(),
      slow,
      slowJar,
    ...
    await capturedRead;
    try {
      const first = await startNotebookServer(
        "supermind",
        page(),
        fast,
        fastJar,
...
test("session startup is opt-in and verifies kernels after ready, without code execution", async () => {
  await isolated(async () => {
    ...
    const transport: Transport = async (url, init) => {
      if (url.endsWith("/hub/login"))
        return response("", 302, { location: "/notebook/hub/spawn" });
      ...
      if (url.endsWith("/api/kernelspecs"))
        return response('{"default":"python3"}');
      throw new Error("Unexpected request");
    };
    const http = new SafeHttp("supermind", jar, transport);
    await assert.rejects(
      notebookSession("supermind", http),
      (error: unknown) =>
        error instanceof CliError && error.code === "SERVER_NOT_READY",
    );
    assert.equal(posts, 0);
    const session = await notebookSession("supermind", http, {
      startServer: true,
      jar,
    });
